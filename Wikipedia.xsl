...
<!--
 | @file
 | Wikipedia Bibliography Style for Microsoft Word 2010
 |
 | Bibliography Style based on the rules of the German Wikipedia for
 | Citations. The Wikipedia styles where slightly altered to fit the
 | needs of printed media when it comes to displaying hyperlinks.
 |
 | A good starting point to understand the Bibliography Style system of
 | Microsoft Word 2010 is the following resource publiched by Microsoft:
 |
 | http://blogs.office.com/b/microsoft-word/archive/2007/12/14/bibliography-citations-1011.aspx
 |
 | Please note that this Bibliography Styles is absolutely unusable
 | without the BIBFORM.XML file which you should get along with this
 | file. Please follow the instructions in the README.md file on what
 | exactly you have to do with that file. If you don't have the
 | BIBFORM.XML file visit the GitHub repository and get it from there:
 |
 | @todo GitHub link
 |
 | ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
 |
 | To the extent possible under law, Richard Fussenegger has waived all
 | copyright and related or neighboring rights to Wikipedia Bibliography
 | Style for Microsoft Word 2010. This work is published from: Austria.
 |
...
<xsl:stylesheet version="1.0"
  ...
  <!-- The generated output is HTML and encoded in utf-8 -->
  <xsl:output method="html" encoding="us-ascii"/>

  <!-- Match the root element, and dispatch to Start -->
  <xsl:template match="/">
  ...
  <xsl:template name="Start">
    <xsl:choose>
      ...
      <xsl:when test="b:Version">
        <xsl:text>2012.0.0</xsl:text>
      </xsl:when>
      <!-- Name of this style -->
      <xsl:when test="b:StyleName">
        <xsl:text>Wikipedia</xsl:text>
      </xsl:when>
      <!-- Category of this style -->
      <xsl:when test="b:category">
        <xsl:text/>
      </xsl:when>

      <!-- Define which fields should be displayed in the dialog -->
      <xsl:when test="b:GetImportantFields">
        <b:ImportantFields>
          <xsl:choose>

            <!--
             | Fields for type "Buch"
             |
             | http://de.wikipedia.org/wiki/Vorlage:Literatur
            -->
            ...
            <!-- Fields for type "Zeitungsartikel" -->
            ...
            <!-- Fields for type "Dokument von Website" -->
            <xsl:when test="b:GetImportantFields/b:SourceType='DocumentFromInternetSite'">
              ...
              <b:ImportantField>
                <xsl:text>b:Date</xsl:text>
              </b:ImportantField>
              <b:ImportantField>
                <xsl:text>b:URL</xsl:text>
              </b:ImportantField>
              <b:ImportantField>
                <xsl:text>b:Comment</xsl:text>
              </b:ImportantField>
              <b:ImportantField>
                <xsl:text>b:Accessed</xsl:text>
              </b:ImportantField>
            </xsl:when>

            <!-- Fields for type "Website" -->
  ...
  <xsl:template name="templ_prop_Colon">:</xsl:template>
  ...
  <xsl:template name="BibDisplayAuthor">
    ...
    <xsl:choose>
      ...
      <!-- For less then five authors -->
      <xsl:when test="$cAuthors>0 and 5>$cAuthors">

        <!-- Loop through all authors and print their names. -->
        <xsl:for-each select="b:Author/b:Author/b:NameList/b:Person">

          <!-- Name of the author -->
          <xsl:choose>
            <xsl:when test="count(b:First)=1">
              <xsl:value-of select="b:First"/>
              <xsl:call-template name="templ_prop_Space"/>
            </xsl:when>
          </xsl:choose>
          ...
          <xsl:choose>
            <xsl:when test="count(b:Middle)=1">
              <xsl:call-template name="splitAuthorSpace">
                <xsl:with-param name="first">
                  <xsl:call-template name="right-trim">
                    <xsl:with-param name="s" select="b:Middle"/>
          ...
          <xsl:choose>
            <xsl:when test="count(b:Last)=1">
              <xsl:value-of select="b:Last"/>
              <xsl:choose>
                <xsl:when test="$cAuthors>1 and (position()+1)=$cAuthors">, </xsl:when>
  ...
  <!--
   ...
   |
   | @return string
   |   Depending on the given fields this will return "[Day]. [Month] [Year]",
   |   "[Month] [Year]" or "[Year]".
   -->
  <xsl:template name="BibDisplayDayMonthYear">
    <!-- [Day]. [Month] [Year] -->
    <xsl:if test="count(b:Day)!=0 and count(b:Month)!=0 and count(b:Year)!=0">
    ...
    <!-- [Month] [Year] -->
    <xsl:if test="count(b:Month)!=0 and count(b:Year)!=0">
      <xsl:value-of select="b:Month"/>
      <xsl:call-template name="templ_prop_Space"/>
    </xsl:if>

    <!-- [Year] -->
    <xsl:if test="count(b:Year)!=0">
      <xsl:value-of select="b:Year"/>
    </xsl:if>

    <!-- @todo extend -->
    <xsl:choose>
      <xsl:when test="count(b:StandardNumber)!=0 or count(b:Pages)!=0">
        <xsl:call-template name="templ_prop_ListSeparator"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:call-template name="templ_prop_Dot"/>
  ...
  <xsl:template name="BibDisplayTitleSupplement">
    <xsl:if test="count(b:TitleSupplement)!=0">
      <xsl:value-of select="b:TitleSupplement"/>
      <xsl:call-template name="templ_prop_Dot"/>
      <xsl:call-template name="templ_prop_Space"/>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template name="BibDisplayInternetSiteTitle">
    <xsl:if test="count(b:InternetSiteTitle)!=0">
      In: <i><xsl:value-of select="b:InternetSiteTitle"/></i>
      <xsl:call-template name="templ_prop_Dot"/>
      <xsl:call-template name="templ_prop_Space"/>
    </xsl:if>
  </xsl:template>

  <xsl:template name="BibDisplayInternetPublisher">
    <xsl:if test="count(b:InternetPublisher)!=0">
      <xsl:value-of select="b:InternetPublisher"/>
      <xsl:call-template name="templ_prop_ListSeparator"/>
    </xsl:if>
  </xsl:template>

  <xsl:template name="BibDisplayDate">
    <xsl:if test="count(b:Date)!=0">
      <xsl:value-of select="b:Date"/>
      <xsl:call-template name="templ_prop_ListSeparator"/>
    </xsl:if>
  </xsl:template>

  <xsl:template name="BibDisplayArchive">
    <xsl:if test="count(b:ArchiveURL)!=0">
      archiviert vom Original unter
      <u>
        <xsl:value-of select="b:ArchiveURL"/>
      </u>
      <xsl:if test="count(b:ArchiveDate)!=0">
        am <xsl:value-of select="b:ArchiveDate"/>
      </xsl:if>
      <xsl:call-template name="templ_prop_ListSeparator"/>
    </xsl:if>
  </xsl:template>

  <xsl:template name="BibDisplayAccessed">
    <xsl:if test="count(b:Accessed)!=0">
      Abgerufen am
      <xsl:value-of select="b:Accessed"/>
    </xsl:if>
  </xsl:template>

  <xsl:template name="BibDisplayFormat">
    <xsl:if test="count(b:Format)!=0">
      <xsl:call-template name="templ_prop_Space"/>
      <xsl:call-template name="templ_prop_OpenBracket"/>
      <xsl:value-of select="b:Format"/>
      <xsl:choose>
        <xsl:when test="count(b:Language)!=0 or count(b:Comments)!=0">
          <xsl:call-template name="templ_prop_ListSeparator"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:call-template name="templ_prop_CloseBracket"/>
  ...
  <xsl:template name="BibDisplayLanguage">
    <xsl:if test="count(b:Language)!=0">
      <xsl:if test="count(b:Format)=0">
        <xsl:call-template name="templ_prop_Space"/>
        <xsl:call-template name="templ_prop_OpenBracket"/>
      </xsl:if>
      <xsl:value-of select="b:Language"/>
      <xsl:choose>
        <xsl:when test="count(b:Comments)!=0">
          <xsl:call-template name="templ_prop_ListSeparator"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:call-template name="templ_prop_CloseBracket"/>
  ...
  <xsl:template name="BibDisplayInternetComment">
    <xsl:if test="count(b:Comments)!=0">
      <xsl:if test="count(b:Format)=0 and count(b:Language)=0">
        <xsl:call-template name="templ_prop_Space"/>
        <xsl:call-template name="templ_prop_OpenBracket"/>
      </xsl:if>
  ...
  <xsl:template name="BibDisplayCitation">
    <xsl:choose>
      <xsl:when test="count(b:Citation)!=0">
        <xsl:call-template name="templ_prop_Colon"/>
        <xsl:call-template name="templ_prop_Space"/>
        <xsl:call-template name="templ_prop_OpenQuote"/>
        <xsl:value-of select="b:Citation"/>
  ...
  <!--
   | Display the name of a journal.
   |
   | @return string
   |   The name of the journal in the format "In: <i>Name</i>.".
   -->
  <xsl:template name="BibDisplayJournalName">
    <xsl:if test="count(b:JournalName)!=0">
      In: <i><xsl:value-of select="b:JournalName"/></i>.
    </xsl:if>
  </xsl:template>
  ...
  <!--
   ...
   |
   | @return string
   |   The city with leading comma or dot; depending on if it is the last
   |   element or not.
   -->
  <xsl:template name="BibDisplayCity">
    <xsl:if test="count(b:City)!=0">
      <xsl:value-of select="b:City"/>
      <xsl:choose>
        <xsl:when test="count(b:Year)!=0">
          <xsl:call-template name="templ_prop_Space"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:call-template name="templ_prop_ListSeparator"/>
  ...
  <!--
   | Display the Standard Number (ISBN).
   |
   | @return string
   |   The given Standard Number in the format "ISBN/ISSN: [StandardNumber]".
   -->
  <xsl:template name="BibDisplayStandardNumber">
    <xsl:if test="count(b:StandardNumber)!=0">
      ISBN/ISSN: <xsl:value-of select="b:StandardNumber"/>
      <xsl:choose>
        <!-- @todo extend -->
        <xsl:when test="count(b:Pages)!=0">
          <xsl:call-template name="templ_prop_ListSeparator"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:call-template name="templ_prop_Dot"/>
  ...
  <!--
   ...
   |
   | @return string
   |   The publisher with e leading comma or dot; depending on if it is the
   |   last element or not.
   -->
  <xsl:template name="BibDisplayPublisher">
    <xsl:if test="count(b:Publisher)!=0">
      <xsl:value-of select="b:Publisher"/>
      <xsl:choose>
        <!-- @todo extend -->
        <xsl:when test="count(b:City)!=0 or count(b:Year)!=0 or count(b:Month)!=0 or count(b:Day) !=0 or count(b:StandardNumber)!=0 or count(b:Pages)!=0">
          <xsl:call-template name="templ_prop_ListSeparator"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:call-template name="templ_prop_Dot"/>
  ...
  <xsl:template name="ProceedBibliography">
    ...
    <xsl:for-each select="msxsl:node-set($sList)/b:Bibliography/b:Source">
      ...
      <xsl:choose>
        ...
        <!--
         | Style for type "Website" based on the Wikipedia template
         | "Literatur".
         |
         | @link http://de.wikipedia.org/wiki/Vorlage:Literatur
         -->
        ...
        <!--
         ...
         | "Internetquelle".
         |
         | @link http://de.wikipedia.org/wiki/Vorlage:Internetquelle
         -->
        <xsl:when test="$SourceType = 'InternetSite'">
          <xsl:element name="p">
            ...
            <xsl:call-template name="BibDisplayTitleSupplement"/>
            <xsl:call-template name="BibDisplayInternetSiteTitle"/>
            <xsl:call-template name="BibDisplayInternetPublisher"/>
            <xsl:call-template name="BibDisplayDate"/>
            <xsl:call-template name="BibDisplayPages"/>
            <xsl:call-template name="BibDisplayArchive"/>
            <xsl:call-template name="BibDisplayAccessed"/>
            <xsl:call-template name="BibDisplayFormat"/>
            <xsl:call-template name="BibDisplayLanguage"/>
            <xsl:call-template name="BibDisplayInternetComment"/>
            <xsl:call-template name="BibDisplayCitation"/>
            <br>
        ...
        <!--
         | Style for type "Dokument von Website" based on the Wikipedia
        ...
        <!--
         | Style for type "Zeitungsartikel" based on the Wikipedia
         | template "Literatur".
         |
         | @link http://de.wikipedia.org/wiki/Vorlage:Literatur
         -->
